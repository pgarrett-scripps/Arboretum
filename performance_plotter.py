...
def generate_random_record() -> PSM:
...
linspace = [(i+1)*num_records for i in range(num_points)]
performance_dict = {}
...
for tree_type in [TreeType.SORTED_LIST, TreeType.HASHTABLE, TreeType.HASHTABLE_LARGE, TreeType.AVL, TreeType.RB, TreeType.BINARY]:
    performance_dict[tree_type] = {'add_time':[], 'add_time_per_record':[], 'search_time':[], 'search_time_per_record':[],
                                   'remove_time':[], 'remove_time_per_record':[], 'save_time':[], 'save_time_per_record':[],
                                   'load_time':[], 'load_time_per_record':[]}
    tree = psm_tree_constructor(tree_type)
    for i in range(num_points):

        psms = [generate_random_record() for j in range(num_records)]

        add_start_time = time.time()
        for psm in psms[:n]:
            tree.add(psm)
        add_time = time.time() - add_start_time
        add_time_per_record = add_time/n

        for psm in psms[n:]:
            tree.add(psm)

        performance_dict[tree_type]['add_time'].append(add_time)
        performance_dict[tree_type]['add_time_per_record'].append(add_time_per_record)
        ...
        search_time_per_record = search_time/n

        performance_dict[tree_type]['search_time'].append(search_time)
        performance_dict[tree_type]['search_time_per_record'].append(search_time_per_record)

        remove_start_time = time.time()
        for psm in psms[:n]:
            _ = tree.remove(psm)
        remove_time = time.time() - remove_start_time
        remove_time_per_record = remove_time/n

        performance_dict[tree_type]['remove_time'].append(remove_time)
        performance_dict[tree_type]['remove_time_per_record'].append(remove_time_per_record)
        ...
        performance_dict[tree_type]['save_time_per_record'].append(save_time/n)
        ...
        performance_dict[tree_type]['load_time_per_record'].append(load_time/n)
...
for tree_type in performance_dict:
    ax1.plot(linspace, performance_dict[tree_type]['add_time_per_record'], label=tree_type)

ax1.title.set_text('Add Time per Record')

for tree_type in performance_dict:
    ax2.plot(linspace, performance_dict[tree_type]['search_time_per_record'], label=tree_type)
ax2.title.set_text('Search Time per Record')

for tree_type in performance_dict:
    ax3.plot(linspace, performance_dict[tree_type]['remove_time_per_record'], label=tree_type)
ax3.title.set_text('Remove Time per Record')
